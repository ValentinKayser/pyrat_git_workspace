...
#####################################################################################################################################################
    ...
    def find_route(self, matrice, graph, initial_location):
        ...
        for s1 in matrice:
            S.append(s1)
        def recherche_greedy(initial_location, S, matrice):
            # Initialisation : sommet de départ et liste des sommets visités
            sommet = [initial_location]
            S.remove(initial_location)  # Retirer le sommet de départ de S
            current = initial_location
            while not (S==[]):
                voisins_non_visites = [k for k in matrice[current] if matrice[current][k] != 0]
                mini_L = min(voisins_non_visites, key=lambda k: matrice[current][k])
                while mini_L in sommet:
                    voisins_non_visites.remove(mini_L)
                    mini_L = min(voisins_non_visites, key=lambda k: matrice[current][k])
                sommet.append( mini_L)
                S.remove( mini_L)
                current =  mini_L
            return sommet


        chemin_valide = recherche_greedy(initial_location, S, matrice)
        chemin_fini = []
